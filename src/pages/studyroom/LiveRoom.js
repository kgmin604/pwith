...
const LiveRoom = () => {
  ...
  const createOffer = async () => {
    ...
    console.log(
      "createOffer setRemoteDescription 시그널 상태",
  ...
  const createAnswer = async (sdp) => {
    ...
    console.log(
      "createAnswer setRemoteDescription 시그널 상태",
      peerRef.current.signalingState
    );
    ...
    console.log(
      "createAnswer setLocalDescription 시그널 상태",
      peerRef.current.signalingState
    );
  ...
  useEffect(() => {
    ...
    socketRef.current.on("answer", async (sdp) => {
      ...
      console.log(
        "on answer setRemoteDescription 시그널 상태",
        peerRef.current.signalingState
      );
      peerRef.current.setRemoteDescription(sdp);
    });
    ...
    <div className="live-room" ref={ref}>
      <div className="left-space">
        <div className="people-list">
          ...
          {data.map((a, i) => {
            return (
              <div className="people">
                <img className="user-data" src={userImg} />
        ...
        {isCodeOn && (
          <div className="code-upload">
            ...
            <textarea placeholder="코드를 업로드 하세요!" />
          </div>
        )}
      </div>
      ...
      {showChat && (
        <div className="right-space">
          ...
          <form
            className={`chat-input ${isClicked ? "clicked" : ""}`}
